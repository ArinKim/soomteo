...
export default function ChatListView({ friends, openChatSession }) {
  return (
    <ScrollView>
      {friends.map((friend) => (
        <TouchableOpacity key={friend.id} style={styles.chatRow} onPress={() => openChatSession(friend)}>
          <View style={[styles.avatarSmall, { backgroundColor: friend.avatarColor || "#ddd" }]}>
