...
숨터의 AI 상담 모델은 **텍스트·음성 기반 감정 분석**, **위험도 판단**, **심리 상담 대화 생성**을 위해
다양한 공개 및 실제 상담 기반 데이터셋을 활용해 학습됩니다.
...
* **감성 대화 말뭉치(KOTE 등)**
  → 긍정·부정 감정 분류, 감정 흐름 분석에 사용
* **웰니스 대화 스크립트 데이터셋**
  → 실제 상담/진료 기록 기반의 공감적 대화 패턴 학습
* **한국어 SNS 멀티턴 대화 데이터셋 / SNS 고도화 데이터**
  → 일상 대화 능력, 멀티턴 자연스러운 발화 흐름 학습
* **아동·청소년 상담 데이터**
  → 청소년이 겪는 고민·감정 상황 학습, 또래 기반 상담 대응 강화
* **심리 상담 데이터(성인·약자 대상)**
  → 세대/상황별 맞춤형 상담 톤 & 단계별 대응 학습

#### 2) **음성 데이터**

* **고령인구 우울증·상담 음성 데이터셋**
  → AI 음성 상담(*23#)에서 감정 변화·우울 신호 탐지
  → 고령층에게 적합한 말투·속도·톤 기반 발화 생성
...
* **Upstage SOLAR / Qwen3 / Llama 3.1 중 하나를 사용**
  → 한국어에 강함, 감정 분석·상담 대화에 최적화
* **하나의 Be:U 상담 모델**로
  → 채팅 & 음성 상담 모두 처리
  → 여러 페르소나 지원(톤/스타일 선택 가능)

### ● **음성 파이프라인 (3단)**

> **사용자 음성 → STT → LLM(Be:U) → TTS → 답변 음성**

* **STT: Faster-Whisper**
  → 모바일 환경에서도 빠르고 정확한 음성 텍스트 변환
* **TTS: StyleTTS2**
  → AI와 통화하고 있다는 느낌을 최소화하는 자연스러운 음성 합성

### ● **훈련 및 최적화**

* **PEFT (Parameter-Efficient Fine-Tuning)**
  → 모바일·엣지 환경에서도 학습된 모델을 가볍게 구동 가능
* **TGI(Text Generation Inference) 서버**
  → 실시간 대화 품질 안정화 및 서버 부하 최적화

### ● **실시간 정보 강화 — RAG 적용**

* 청소년 유행/트렌드처럼
  **빠르게 변하는 정보는 RAG로 바로 반영**
* 모델이 모르는 내용 → 빠른 검색/추론 → 최신 답변 제공

### ● **위험도 감지 모델**

* 텍스트 + 음성 기반
  → 감정 악화, 자해 암시, 고위험 발화 실시간 탐지
* 단계별 경고 + 보호자/기관 자동 연결(사전 동의 시)
...
* **STT / TTS 파이프라인** — 음성 상담 서비스
* **GPS 연동** — 위기 발생 시 지역별 센터 안내
* **공공 API 연계** — 정신건강·상담 기관 정보 제공
* **AI 백엔드(확장 예정)** — 감정 분석, 위험 탐지, 세션 관리
