...
// CallingScreen: STT 결과를 문장 단위로 숨겨진 채팅방에 전송
// props
//   friend: 통화중인 친구 객체
...
export default function CallingScreen({ friend, userId, onEndCall }) {
  ...
  // 숨겨진 채팅방 생성 (AI 친구 조건: friend.prompt 존재한다고 가정)
  const createHiddenChatRoom = async () => {
    if (!friend || !userId) return;
    // AI 친구가 아닌 경우 방 생성하지 않음
    if (!friend.prompt) {
      console.log("AI 프롬프트 없음: 숨겨진 채팅방 생성 건너뜀");
      return;
    }
    try {
      console.log("▶ 숨겨진 채팅방 생성 시도", friend.id, userId);
      const res = await fetch(`${API_BASE_URL}/api/chats/hidden`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId, friendId: friend.id }),
      });
      if (!res.ok) {
        console.warn("숨겨진 채팅방 생성 실패", res.status);
        setChatInitError(`failed (${res.status})`);
        return;
      }
      const data = await res.json();
      // 서버가 { roomId } 형태로 내려준다고 가정
      setChatRoomId(data.roomId);
      console.log("✅ 숨겨진 채팅방 생성 성공 roomId=", data.roomId);
    } catch (e) {
      console.warn("숨겨진 채팅방 생성 오류", e);
      setChatInitError("network-error");
    }
  };
  ...
  const sendChatMessage = async (text) => {
    if (!chatRoomId) {
      console.log("chatRoomId 없음: 메시지 전송 건너뜀");
      return;
    }
    if (!userId) {
      console.log("userId 없음: 메시지 전송 불가");
      return;
    }
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/chats/${chatRoomId}/messages`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ fromUserId: userId, text: trimmed }),
        }
      );
      if (!res.ok) {
        console.warn("메시지 전송 실패", res.status);
        return;
      }
      console.log("✉️ 전송 완료:", trimmed);
    } catch (e) {
      console.warn("메시지 전송 오류", e);
    }
  };
  ...
  // 컴포넌트 마운트/언마운트 시 STT + 숨겨진 채팅방 초기화
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const onSpeechResults = (e) => {
      ...
      // 서버에 최종 문장 전송 시도 (숨겨진 채팅방이 있는 경우)
